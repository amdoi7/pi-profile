...
ctx: 53k 20% $0.87 miss 143k (2×)       │ ↑1k ↓317 (τ39%) ℂ99.2%                │ 100 t/s ttfb5.5s 本轮16m21s
```
...
- 行2 尾段（轮级动态）：吞吐 `t/s`（output 含 thinking，墙钟口径）、`ttfb`、`本轮时长`。
...
- `custom-footer-tps.ts` — **轮级 tracker（一条消息 → 一轮）**：本轮（agent_start → agent_settled）时长、t/s、ttfb、以及轮级 token flow（↑↓τℂ 段数据源，`getRoundFlow`）；双消息源——实时源（进行中显示）用 message_end 增量，锁定源（settled 锁定）用 agent_end 批量消息（官方源，失败/aborted 消息无 message_end 也不漏计）。**不含 cost**——成本是 session 级指标。
