 ...
 *
 * Source: https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/core/cache-stats.ts
 * Commit:  9993c96907bb0c97260d2c353c31a3464f211122
 * Version: @earendil-works/pi-coding-agent 0.84.3 (installed dist matches this source)
 *
 * Why vendored: the package exports map only exposes the top-level entry, so a
 * bare-specifier deep import (`@earendil-works/pi-coding-agent/dist/core/cache-stats.js`)
 * fails with ERR_MODULE_NOT_FOUND. Copying the official algorithm unchanged keeps
 * this extension on the upstream semantics for cache-waste accounting (incl. the
 * idleMs / modelChanged attribution fields) without maintaining a parallel
 * reimplementation. Update in place when upstream changes; the local dist
 * comparison in the extension tests guards drift.
 *
 * Import rewrite: `SessionEntry` comes from the official package exports
 * (`./core/session-manager.ts` types are re-exported at the top level); the
 * original relative import is resolved through the same public surface.
 */
