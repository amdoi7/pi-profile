/**
 * CJK/ASCII 标点的等价类:match 里把 `、` 写成 `,`、把 `，` 写成 `,` 这类转写漂移
 * 直接命中,不报错、不占往返。弯引号折叠是同一先例,这里只是把等价类补齐,
 * 并补上一条更强的保序规则。
 *
 * 铁律:模糊命中后不得整段覆盖。模型**无意改动**的部分必须保留文件原字节;
 * 判据:match 与 new_str 的公共前后缀 = 模型无意改的部分 → 取文件字节;中间那段
...
test("a halfwidth comma in the match matches the file's ideographic comma", () => {
	const content = [
	...
	const { newContent } = applyOpToNormalizedContent(content, replace(needle, replacement));
...
test("fullwidth punctuation in the file matches halfwidth in the match", () => {
	const content = ["取值范围（含边界）：0～100；超出即拒绝！", ""].join("\n");
	const needle = "取值范围(含边界):0～100;超出即拒绝!";
...
// 边界：全角空格参与**匹配**（match 写成普通空格也能命中），但不参与**回写**。
...
// 多命中不进修复：「每一处」里各处的字节形式可能不同，修成一种会漏其余。
