 ...
 *
 * 设计边界（重新设计的 review 实证过）：CJK 标点与全角空格**不在**这个等价类，
 * 只在修复面（anchor-alignment 的 SAME_MARK_VARIANTS）折叠。两层刻意的窄→宽：
 * 匹配面无碰撞面，修复面逐步过等长+变体校验、多处可修就整体拒绝（不猜）。
 * 立项证据（修复面）：92 个可修复失败锚里 17 个是 1-2 字符漂移，其中
 * `,`↔`，` 占 4 例。弯引号归一化的失效条件：DUPLICATE_MATCH 里出现因折叠
 * 而多命中的案例，或出现因折叠而改错位置的事故。
 *
 * 不入等价类（证据上就不是同一回事，折了就是改错地方）：
 * - CJK 标点（，、：等）——只在修复面折叠，且要过等长+变体校验；
 * - 汉字形近误写（骨→骰）——语料里它们是失败锚的一大类，必须继续失败；
 * - 破折号 `—`/`–` 与 `-`：`---` 在 Markdown 里是 frontmatter/分割线；
 * - 缩进类空白：Python 里缩进就是语义。
 *
