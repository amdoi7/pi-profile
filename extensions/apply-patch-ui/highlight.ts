 ...
 *
 * 供 apply-patch-ui 的 bash renderCall 回退路径使用：
...
interface Seg {
  text: string;
  color: string | null; // theme token 名；null = 默认前景色
}
  ...
  for (const s of segs) {
    if (raw + s.text.length > maxRaw) {
      ...
      if (take > 0) {
        ...
        out += s.color ? theme.fg(s.color as never, clipped) : clipped;
      }
      out += theme.fg("dim", "…");
      return out;
    }
    out += s.color ? theme.fg(s.color as never, s.text) : s.text;
    raw += s.text.length;
  }
  return out;
}
