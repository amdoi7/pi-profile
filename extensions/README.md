...
- `command-policy/` — `bash` / `run_experiment` 的唯一 `tool_call` mutation owner；依次执行 RTK rewrite、uv Python normalization 与 package-manager deny。
- `rtk/` — RTK pure rewrite policy 与 focused tests；不再直接注册 runtime hook。
